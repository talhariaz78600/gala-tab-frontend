...
const EditProfileInformation = ({ mode = "edit" }) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await updateProfile(cleanedPayload).unwrap();
      if (response?.data) {
  ...
  useEffect(() => {
    if (currentUser?.country && countriesData?.data?.length) {
      const matchedCountry = countriesData.data.find(
        (country) => country.id === currentUser.country
      );

      setCountrySelect(matchedCountry?.country || "");
    }
  }, [currentUser?.country, countriesData?.data]);
  ...
  return (
    <div className="h-full">
        ...
        <div className="flex flex-col justify-between h-full">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h4 className="font-semibold text-[24px] text-[#000000] dark:text-white">
                Profile Information
              </h4>
            </div>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <PhoneInput
                      ...
                      className={`bg-white text-black shadow-lg rounded-lg py-1 px-4 w-full ${
      ...
      <Loader
        loading={updatingProfile || isLoading || countryLoading || cityLoading}
      />
    </div>
  );
};
