...
export default function ManageCity() {
  ...
  const [deleteCity, { isLoading: deleteLoading }] = useCityDeleteMutation();
  ...
  const handleStatusToggle = async (value) => {
    ...
    try {
      const response = await cityUpdate({
        id: value._id,
  ...
  return (
    <div className="bg-[#F7F7F7] dark:bg-[#1E1E1E] min-h-[calc(100vh-130px)] rounded-[20px]">
      <div className="flex items-center gap-4 flex-wrap justify-between p-5 border-b">
        <p className="text-[28px] leading-normal font-semibold">Manage City</p>
        <div className="ms-auto">
          <Link
            to="/admin-dashboard/add-city"
            className="font-medium border inline-block border-black p-3 bg-white dark:bg-gray-800 rounded-[8px] shadow-[0px_10px_20px_0px_#0000001A]"
          >
            Add City
          </Link>
        </div>
            ...
            },
            {
              ...
              data: (value, task) => (
                <div className="flex items-center gap-2">
                  <button
                    onClick={() =>
                      navigate(`/admin-dashboard/edit-City/${task._id}`, {
                        state: { data: task },
                      })
                    }
                  >
